...
def convert_shape(np_array):
    # convert the shape of input array
    # before: 5 * 660
    # after: 50 * 66
    # print("original shape: {}".format(np_array.shape))

    list_reshape = []
    for i in range(10):
        temp_5_66 = np_array[:, 0+66*i:66+66*i]
        list_reshape.append(temp_5_66)
    np_reshape = np.array(list_reshape).reshape(50,66)
    # print("after reshaped: {}".format(np_reshape.shape))
    return np_reshape

def load_input(path_parent, list_hypnos):
    data = []
    for sub_ID in tqdm(list_hypnos, desc='Load data ... '):
        temp_data = pd.read_csv(os.path.join(path_parent, sub_ID, 'probabilistic_hypnogram.csv'), index_col=0)
        temp_data = temp_data[['W', 'R', 'N1', 'N2', 'N3']].transpose()
        temp_data = temp_data.values[:,:,np.newaxis]
        data.append(temp_data)
    data = np.array(data)
    data = convert_shape(data)
    return data
